<script lang="ts">
    ...
    let profiles: ProfileManagementEntity[] = [];
    let selected_profile: ProfileManagementEntity | null = null;
    let is_loading = true;
    let error_message = "";
    let related_entity_options: ProfileManagementOption[] = [];
    let permissions: ProfileManagementPermissions =
        build_profile_management_permissions(
            undefined,
    ...
    function handle_preview_click(entity: ProfileManagementEntity): void {
        ...
        if (!browser || !current_preview_path) {
            return;
        }
    ...
    async function handle_delete_click(
        ...
        if (!confirm(configuration.delete_confirmation_message)) {
            return;
        }
    ...
    onMount(async () => {
        ...
        if (!permissions.can_read) {
            ...
            goto(PROFILE_MANAGEMENT_PAGE_MESSAGES.HOME_ROUTE);
            return;
        }
