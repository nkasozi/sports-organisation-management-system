<script lang="ts">
    ...
    import GameManageEventModal from "$lib/presentation/components/game/GameManageEventModal.svelte";
    ...
    async function load_fixture(): Promise<void> {
        is_loading = true;
        error_message = "";
        const result = await load_game_manage_bundle(fixture_id, {
            fixture_use_cases,
            team_use_cases,
            player_use_cases,
        });
