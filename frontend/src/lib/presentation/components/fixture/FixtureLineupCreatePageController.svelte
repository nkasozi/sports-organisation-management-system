<script lang="ts">
    ...
    let derived_state = build_fixture_lineup_create_page_derived_state(get_derived_state_input());

    $: current_auth_profile =
        $auth_store.current_profile as UserScopeProfile | null;
    $: derived_state = build_fixture_lineup_create_page_derived_state(get_derived_state_input());
    $: {
    ...
    $: if (
        ...
        selected_organization = sync_fixture_lineup_create_selected_organization(form_data.organization_id, organizations);
    const runtime = create_fixture_lineup_create_page_controller_runtime({
        ...
        get_fixtures_with_complete_lineups: () => fixtures_with_complete_lineups,
        get_form_data: () => form_data,
        get_max_players: () => max_players,
        get_min_players: () => min_players,
        get_organization_is_restricted: () => derived_state.organization_is_restricted,
        ...
        set_confirm_lock_understood: (value: boolean) => (confirm_lock_understood = value),
        set_current_step_index: (value: number) => (current_step_index = value),
        set_error_message: (value: string) => (error_message = value),
        set_fixture_team_label_by_team_id: (value: Map<string, string>) => (fixture_team_label_by_team_id = value),
        set_fixtures: (value: Fixture[]) => (fixtures = value),
        set_fixtures_with_complete_lineups: (value: Set<string>) => (fixtures_with_complete_lineups = value),
        ...
        set_selected_fixture: (value: Fixture | null) => (selected_fixture = value),
        set_selected_organization: (value: Organization | null) => (selected_organization = value),
        ...
        set_teams_with_existing_lineups: (value: Map<string, string>) => (teams_with_existing_lineups = value),
        set_validation_errors: (value: Record<string, string>) => (validation_errors = value),
    });
...
<FixtureLineupCreatePageView
    ...
    fixtures_for_organization_count={derived_state.fixtures_for_organization.length}
    ...
    existing_lineup_team_names={Array.from(teams_with_existing_lineups.values())}
    ...
    on_fixture_change={(fixture_id) => void runtime.handle_fixture_change(fixture_id)}
    on_team_change={(team_id) => void runtime.handle_team_change(team_id)}
    on_player_search_change={(search_text) => (player_search_text = search_text)}
    ...
    get_player_role_label={(player_index) => get_player_role_label(player_index, starters_count)}
