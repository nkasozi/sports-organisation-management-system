...
{#if selected_event_type}
    <div class="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-0 sm:items-center sm:p-4">
        <div class="w-full bg-white shadow-2xl dark:bg-gray-800 sm:max-w-md sm:rounded-xl">
            <div class="flex items-center justify-between border-b border-gray-200 p-4 dark:border-gray-700">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <span class={`text-xs ${selected_team_side === "home" ? "text-blue-600" : "text-red-600"}`}>{selected_team_side === "home" ? `🏠 ${home_team_name}` : `✈️ ${away_team_name}`}</span>
                    </div>
                </div>
                <button aria-label="Close event form" class="rounded p-1 hover:bg-gray-100 dark:hover:bg-gray-700" on:click={on_cancel}>
            ...
            <div class="p-4 space-y-4">
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button type="button" class="rounded-lg bg-gray-100 px-3 py-2 text-sm text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600" on:click={() => (event_minute = Math.floor(game_clock_seconds / 60))}>Reset to {Math.floor(game_clock_seconds / 60)}'</button>
                    </div>
                </div>

                {#if selected_event_type.requires_player}
                    <div>
                        <label for="event_player" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Player</label>
                        <div class="relative">
                            <input
                                id="event_player"
                                type="text"
                            ...
                            {#if show_player_dropdown && filtered_players.length > 0}
                                <div class="absolute z-10 mt-1 max-h-64 w-full overflow-y-auto rounded-lg border border-gray-300 bg-white shadow-lg dark:border-gray-600 dark:bg-gray-700">
                                        ...
                                        >
                                            <div class="flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <div class="text-xs text-gray-500 dark:text-gray-400">{player.position || "No position"}</div>
                                                </div>
                                                <div class="text-xs text-gray-400 dark:text-gray-500">{player.status}</div>
                                            </div>
                                        </button>
                                    {/each}
                                </div>
            ...
            <div class="flex justify-end gap-3 border-t border-gray-200 p-4 dark:border-gray-700">
                <button class="px-4 py-2 text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200" on:click={on_cancel}>Cancel</button>
                <button
