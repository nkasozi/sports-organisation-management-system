...
export function cancel_pending_debounce(): void {
  if (sync_state.debounce_timer.status !== "scheduled") return;
  clearTimeout(sync_state.debounce_timer.timer_id);
  sync_state.debounce_timer = { status: "idle" };
}
...
export async function execute_push_sync(): Promise<boolean> {
  ...
  if (sync_deps.orchestrator.status !== "configured") {
    console.warn("[BackgroundSync] No orchestrator — skipping sync");
    return false;
  }

  const timestamp_cache =
    sync_deps.remote_subscriber.status === "configured"
      ? sync_deps.remote_subscriber.value.get_cached_table_timestamps()
      : void 0;
  console.log("[BackgroundSync] Executing push-only sync...");
  const result = await sync_deps.orchestrator.value.sync_now("push", {
...
export async function run_network_restoration_sync(
  ...
  const websocket_was_running =
    sync_deps.remote_subscriber.status === "configured"
      ? sync_deps.remote_subscriber.value.is_running()
      : false;
  ...
  if (websocket_was_running) {
    ...
    if (sync_deps.restoration_handlers.status === "configured") {
      sync_deps.restoration_handlers.value.stop_remote_sync();
    }
  }

  try {
    const result = await sync_deps.orchestrator.value.sync_now(
      "bidirectional",
      {
    ...
    if (websocket_was_running) {
      ...
      if (sync_deps.restoration_handlers.status === "configured") {
        sync_deps.restoration_handlers.value.start_remote_sync();
      }
    }
...
export function start_offline_retry_timer(): void {
  if (sync_state.offline_retry_timer.status === "scheduled") return;
...
export function stop_offline_retry_timer(): void {
  if (sync_state.offline_retry_timer.status !== "scheduled") return;
  clearInterval(sync_state.offline_retry_timer.timer_id);
  sync_state.offline_retry_timer = { status: "idle" };
}

export function start_scheduled_sync_timer(): void {
  if (sync_state.scheduled_sync_timer.status === "scheduled") return;
...
export function stop_scheduled_sync_timer(): void {
  if (sync_state.scheduled_sync_timer.status !== "scheduled") return;
  clearInterval(sync_state.scheduled_sync_timer.timer_id);
  sync_state.scheduled_sync_timer = { status: "idle" };
